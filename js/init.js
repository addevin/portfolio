jQuery(document).ready(function () { "use strict"; dizme_tm_modalbox(); dizme_tm_nav_bg(); dizme_tm_trigger_menu(); dizme_tm_service_popup(); dizme_tm_modalbox_news(); dizme_tm_modalbox_portfolio(); progress_by_frenify(); dizme_tm_mycounter(); dizme_tm_projects(); dizme_tm_portfolio(); dizme_tm_cursor(); dizme_tm_imgtosvg(); dizme_tm_popup(); dizme_tm_data_images(); dizme_tm_contact_form(); newsletter_form(); dizme_tm_owl_carousel(); dizme_tm_input_padding(); dizme_tm_totop(); dizme_tm_down();  jQuery(window).load('body', function () { dizme_tm_my_load();  }); jQuery(window).on('scroll', function () { dizme_tm_progress_line(); }); }); function dizme_tm_modalbox() { "use strict"; jQuery('.dizme_tm_all_wrap').prepend('<div class="dizme_tm_modalbox"><div class="box_inner"><div class="close"><a href="#"><i class="icon-cancel"></i></a></div><div class="description_wrap"></div></div></div>'); }
...
function dizme_tm_contact_form() {
    "use strict"; jQuery(".contact_form #send_message").on('click', function () {
        var formLoader = jQuery(".contact_form #send_message");
        formLoader.html("<span>Sending data...</span>");
        formLoader.attr("disabled","disabled");
        ...
        var emailValidation = IsEmail(email);
        if (name === '' || email === '' || message === '' || emailValidation ===false) {
            jQuery('.contact_form div.empty_notice').slideDown(500).delay(2000).slideUp(500);
            formLoader.html("<span>Submit Now</span>");

        }
        else {
            jQuery.post("https://script.google.com/macros/s/AKfycbzOSwSb0Xez6Bls5tGDXQStPk9rJnSUH-GRRG5qhRpIGW-dJGE4qkwVAnVKzCiPFNfZ/exec",
                { USER_NAME: name, USER_EMAIL: email, USER_PHONE: phone, USER_MESSGE: message, MESSAGE_SUBJECT: subject },
                function (data) {
                    jQuery(".contact_form .returnmessage").append(data);
                    if (jQuery(".contact_form .returnmessage span.contact_error").length) {
                        jQuery(".contact_form .returnmessage").slideDown(500).delay(20000).slideUp(500);
                        formLoader.html("<span>Submit Now</span>");
                    publicModal("Ooops! somthing went wrong!", "Please check the values and try again! ", "error");


                    } else {
                        jQuery(".contact_form .returnmessage").append("<span class='contact_success'>" + success + "</span>");
                        jQuery(".contact_form .returnmessage").slideDown(500).delay(40000).slideUp(500);
                        jQuery("#contact_form")[0].reset();
                        formLoader.html("<span>Submit Now</span>");
                    publicModal("Success!", success, "success");
...
function newsletter_form() {
    "use strict"; jQuery(".newsletter #emailLetterSubmit").on('click', function () {
        var formLoader = jQuery(".newsletter #emailLetterSubmit");
        formLoader.val("Adding...");
        var email = jQuery(".newsletter #newsletterEmail").val();
        var success = jQuery(".newsletter .returnmessage").data('success');
        jQuery(".newsletter .returnmessage").empty();
        var emailValidation = IsEmail(email);
        if (email === '' || emailValidation===false) {
            // jQuery('div.empty_notice').slideDown(500).delay(2000).slideUp(500);
            publicModal("Ooops! somthing went wrong!", "Please check the values and try again! ", "error");
            
            formLoader.val("Send Now");

        }
        else {
            jQuery.post("https://script.google.com/macros/s/AKfycbxuMM_5AE50fvnNAPvny1Go12M15WSQGDie2cu-V7NN6mIKj6hAYqxhVyQj7D0UytrAXg/exec",
                { USER_Newsletter_EMAIL: email },
                function (data) {
                    jQuery(".newsletter .returnmessage").append(data);
                    if (jQuery(".newsletter .returnmessage span.contact_error").length) {
                        // jQuery(".newsletter .returnmessage").slideDown(500).delay(20000).slideUp(500);
                        formLoader.val("Send Now");
                        
                    publicModal("Ooops! somthing went wrong!", "Please check the values you have entered and try again! ", "error");

                    } else {
                        // jQuery(".newsletter .returnmessage").append("<span class='contact_success'>" + success + "</span>");
                        // jQuery(".newsletter .returnmessage").slideDown(500).delay(40000).slideUp(500);
...
function publicModal(mod_head, mod_txt, mod_status) {
    $("#publicModalAd").css("display","block");
    $("#modalHeadData").html('<span class="'+mod_status+'">'+mod_head+'</span>');
    $("#modalData").html('<span class="'+mod_status+'">'+mod_txt+'</span>');


}
function checkEmailIsGivenOrNot(thisid) {
    if(localStorage.getItem("useremail") === null){
        var useremail = localStorage.getItem("useremail");
    }else{
        var useremail = localStorage.getItem("useremail");
        publicModal("Confirm", "You have already submitted an email for newsletter, and just ignore if you wish to provid us new email.</br> Email: "+useremail, " ");
        $(thisid).removeAttr("onclick");
